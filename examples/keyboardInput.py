...
while myWin.isOpen:
	...
	if myWin.keyIsDown( 'Left' ) and (character.x > 0):
		character.x -= 4
		
	if myWin.keyIsDown( 'Right') and (character.x < myWin.width - 40):
		character.x += 4
		
	if myWin.keyIsDown( 'Up'   ) and (character.y > 0):
		character.y -= 4
		
	if myWin.keyIsDown( 'Down' ) and (character.y < myWin.height - 40):
		character.y += 4
